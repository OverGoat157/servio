<script setup>
import { useRoute, useRouter } from 'vue-router'
import { restaurant, loading, error } from '../stores/restaurant'

const route = useRoute()
const router = useRouter()
const slug = route.params.slug

function goToMenu() {
  router.push({ name: 'menu', params: { slug } })
}
</script>

<template>
  <div class="home">
    <!-- Loading -->
    <div v-if="loading" class="loading-screen">
      <div class="spinner"></div>
    </div>

    <!-- Error -->
    <div v-else-if="error" class="error-screen">
      <div class="error-icon">
        <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
          <circle cx="12" cy="12" r="10"/><path d="M12 8v4M12 16h.01"/>
        </svg>
      </div>
      <p class="error-title">Ресторан не найден</p>
      <p class="error-sub">Проверьте правильность ссылки</p>
    </div>

    <!-- Content -->
    <template v-else>
      <!-- Hero — full screen -->
      <div class="hero">
        <div class="hero-overlay"></div>
        <div class="hero-content">
          <div class="hero-top">
            <div class="logo" v-if="restaurant.logo">
              <img :src="restaurant.logo" :alt="restaurant.name" />
            </div>
            <div class="logo logo-letter" v-else>
              {{ restaurant.name.charAt(0) }}
            </div>
          </div>

          <div class="hero-center">
            <h1>{{ restaurant.name }}</h1>
            <p class="tagline" v-if="restaurant.description">{{ restaurant.description }}</p>
          </div>

          <div class="hero-bottom">
            <div class="hours-inline" v-if="restaurant.working_hours">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
                <circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/>
              </svg>
              <span v-for="(time, day) in restaurant.working_hours" :key="day">
                {{ day }}: {{ time }}
              </span>
            </div>
          </div>
        </div>
      </div>

      <!-- Info section -->
      <div class="info-section">
        <!-- Address card -->
        <div class="info-card" v-if="restaurant.address">
          <div class="info-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--text-secondary)" stroke-width="2" stroke-linecap="round">
              <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/>
            </svg>
          </div>
          <div>
            <div class="info-label">Адрес</div>
            <div class="info-value">{{ restaurant.address }}</div>
          </div>
        </div>

        <!-- Phone card -->
        <a class="info-card" v-if="restaurant.phone" :href="'tel:' + restaurant.phone">
          <div class="info-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--text-secondary)" stroke-width="2" stroke-linecap="round">
              <path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6A19.79 19.79 0 012.12 4.11 2 2 0 014.11 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/>
            </svg>
          </div>
          <div>
            <div class="info-label">Телефон</div>
            <div class="info-value">{{ restaurant.phone }}</div>
          </div>
        </a>

        <!-- Working hours card -->
        <div class="info-card hours-card" v-if="restaurant.working_hours">
          <div class="info-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--text-secondary)" stroke-width="2" stroke-linecap="round">
              <circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/>
            </svg>
          </div>
          <div class="hours-full">
            <div class="info-label">Режим работы</div>
            <div class="hours-grid">
              <div class="hours-row" v-for="(time, day) in restaurant.working_hours" :key="day">
                <span class="day">{{ day }}</span>
                <span class="time">{{ time }}</span>
              </div>
            </div>
          </div>
        </div>
      </div>

      <!-- CTA Button -->
      <div class="cta-section">
        <button class="btn-menu" @click="goToMenu">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
            <path d="M4 6h16M4 12h16M4 18h16"/>
          </svg>
          Смотреть меню
        </button>

        <a v-if="restaurant.phone" :href="'tel:' + restaurant.phone" class="btn-call">
          Позвонить
        </a>
      </div>

      <!-- Messengers -->
      <div class="messengers" v-if="restaurant.messengers?.length">
        <div class="msg-label">Мы в мессенджерах</div>
        <div class="msg-list">
          <div class="msg-chip" v-for="m in restaurant.messengers" :key="m">
            <svg v-if="m === 'whatsapp'" width="18" height="18" viewBox="0 0 24 24" fill="#25D366"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.127.556 4.12 1.525 5.855L0 24l6.335-1.652A11.94 11.94 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0z"/></svg>
            <svg v-else-if="m === 'telegram'" width="18" height="18" viewBox="0 0 24 24" fill="#229ED9"><path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.562 8.161c-.18 1.897-.962 6.502-1.359 8.627-.168.9-.5 1.201-.82 1.23-.697.064-1.226-.46-1.9-.903-1.056-.692-1.653-1.123-2.678-1.799-1.185-.781-.417-1.21.258-1.911.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.479.33-.913.492-1.302.484-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.831-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635.099-.002.321.023.465.141a.506.506 0 01.171.325c.016.093.036.306.02.472z"/></svg>
            {{ m === 'whatsapp' ? 'WhatsApp' : m === 'telegram' ? 'Telegram' : m === 'viber' ? 'Viber' : m }}
          </div>
        </div>
      </div>

      <!-- Footer -->
      <footer class="footer">
        <p>Сделано на <a href="https://ab-team.ru" target="_blank">AB Team</a></p>
      </footer>
    </template>
  </div>
</template>

<style scoped>
.home {
  min-height: 100vh;
  display: flex;
  flex-direction: column;
}

/* Loading */
.loading-screen {
  display: flex;
  align-items: center;
  justify-content: center;
  min-height: 100vh;
}

.spinner {
  width: 36px;
  height: 36px;
  border: 3px solid var(--border);
  border-top-color: var(--primary);
  border-radius: 50%;
  animation: spin 0.7s linear infinite;
}

@keyframes spin { to { transform: rotate(360deg); } }

/* Error */
.error-screen {
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
  min-height: 100vh;
  gap: 12px;
  color: var(--text-secondary);
  padding: 24px;
}

.error-icon {
  width: 64px;
  height: 64px;
  border-radius: 50%;
  background: var(--bg-tertiary);
  display: flex;
  align-items: center;
  justify-content: center;
  color: var(--text-secondary);
  margin-bottom: 8px;
}

.error-title {
  font-size: 18px;
  font-weight: 600;
  color: var(--text);
}

.error-sub {
  font-size: 14px;
}

/* Hero */
.hero {
  position: relative;
  min-height: 100svh;
  display: flex;
  align-items: center;
  justify-content: center;
  background: var(--bg);
  overflow: hidden;
}

.hero-overlay {
  position: absolute;
  inset: 0;
  background:
    radial-gradient(ellipse at 30% 20%, rgba(33,103,199,0.06) 0%, transparent 60%),
    radial-gradient(ellipse at 70% 80%, rgba(33,103,199,0.04) 0%, transparent 60%);
}

.hero-content {
  position: relative;
  z-index: 1;
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: space-between;
  min-height: 100svh;
  width: 100%;
  padding: 60px 24px 40px;
}

.hero-top {
  display: flex;
  justify-content: center;
}

.logo {
  width: 88px;
  height: 88px;
  border-radius: 50%;
  overflow: hidden;
  box-shadow: var(--shadow);
}

.logo img {
  width: 100%;
  height: 100%;
  object-fit: cover;
}

.logo-letter {
  background: var(--primary);
  color: var(--primary-foreground);
  display: flex;
  align-items: center;
  justify-content: center;
  font-size: 36px;
  font-weight: 700;
}

.hero-center {
  text-align: center;
  max-width: 380px;
}

.hero-center h1 {
  font-size: 32px;
  font-weight: 700;
  color: var(--text);
  letter-spacing: -0.5px;
  line-height: 1.2;
}

.tagline {
  font-size: 15px;
  color: var(--text-secondary);
  margin-top: 12px;
  line-height: 1.5;
}

.hero-bottom {
  width: 100%;
}

.hours-inline {
  display: flex;
  align-items: center;
  justify-content: center;
  flex-wrap: wrap;
  gap: 6px;
  font-size: 13px;
  color: var(--text-muted);
}

.hours-inline svg {
  flex-shrink: 0;
}

.hours-inline span::after {
  content: ' · ';
}

.hours-inline span:last-child::after {
  content: '';
}

/* Info Section */
.info-section {
  padding: 0 20px;
  display: flex;
  flex-direction: column;
  gap: 1px;
  background: var(--border);
  margin: 0 20px;
  border-radius: var(--radius);
  overflow: hidden;
}

.info-card {
  display: flex;
  align-items: flex-start;
  gap: 14px;
  padding: 16px;
  background: var(--bg);
  text-decoration: none;
  color: inherit;
}

.info-card:only-child {
  border-radius: var(--radius);
}

.info-icon {
  width: 36px;
  height: 36px;
  border-radius: 10px;
  background: var(--bg-secondary);
  display: flex;
  align-items: center;
  justify-content: center;
  flex-shrink: 0;
  margin-top: 2px;
}

.info-label {
  font-size: 11px;
  font-weight: 600;
  text-transform: uppercase;
  letter-spacing: 0.8px;
  color: var(--text-muted);
  margin-bottom: 4px;
}

.info-value {
  font-size: 14px;
  color: var(--text);
  line-height: 1.4;
}

.hours-full {
  flex: 1;
}

.hours-grid {
  display: flex;
  flex-direction: column;
  gap: 6px;
}

.hours-row {
  display: flex;
  justify-content: space-between;
  font-size: 14px;
}

.hours-row .day {
  color: var(--text-secondary);
}

.hours-row .time {
  font-weight: 500;
  color: var(--text);
}

/* CTA */
.cta-section {
  padding: 24px 20px;
  display: flex;
  flex-direction: column;
  gap: 10px;
}

.btn-menu {
  display: flex;
  align-items: center;
  justify-content: center;
  gap: 10px;
  width: 100%;
  padding: 16px;
  background: var(--primary);
  color: var(--primary-foreground);
  border-radius: var(--radius);
  font-size: 16px;
  font-weight: 600;
  transition: opacity var(--ease);
}

.btn-menu:active {
  opacity: 0.85;
}

.btn-call {
  display: flex;
  align-items: center;
  justify-content: center;
  width: 100%;
  padding: 14px;
  background: var(--bg);
  color: var(--text);
  border: 1.5px solid var(--border);
  border-radius: var(--radius);
  font-size: 15px;
  font-weight: 500;
  transition: border-color var(--ease);
}

.btn-call:active {
  border-color: var(--primary);
}

/* Messengers */
.messengers {
  padding: 0 20px 24px;
}

.msg-label {
  font-size: 11px;
  font-weight: 600;
  text-transform: uppercase;
  letter-spacing: 0.8px;
  color: var(--text-muted);
  margin-bottom: 12px;
}

.msg-list {
  display: flex;
  gap: 8px;
}

.msg-chip {
  display: flex;
  align-items: center;
  gap: 8px;
  padding: 10px 16px;
  background: var(--bg-secondary);
  border-radius: 100px;
  font-size: 13px;
  font-weight: 500;
  color: var(--text-secondary);
}

/* Footer */
.footer {
  padding: 32px 20px 40px;
  text-align: center;
}

.footer p {
  font-size: 12px;
  color: var(--text-muted);
}

.footer a {
  color: var(--text-secondary);
  font-weight: 500;
}
</style>
